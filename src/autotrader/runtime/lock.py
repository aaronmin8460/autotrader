"""C8: the single-instance lock. Two runners on one database is duplicate trading.
...
def lock_path_for(database: str | Path) -> Path:
    """The lock file that guards one database.
    ...
    def acquire(self) -> None:
        ...
        try:
            ...
            raise RuntimeLockError(
                f"Another crypto runtime already holds {self.path}. Refusing to start a "
...
__all__ = ["LOCK_SUFFIX", "RuntimeLock", "RuntimeLockError", "lock_path_for"]
