"""Alias-scored V3 drive for the U45∖U30 symbols (ledger §L13).

Same validated mechanism as the prior program's drive (frame scored under the
whitelisted SPY alias, records relabelled back; alias-invariance proven
whole-record against QQQ's stored series), with this program's artifact
isolation: decisions land in this program's dataset root and the log in this
program's report root. All prior dataset directories stay read-only.

Usage:
    python -m studies.equity_asset_character.run_v3_drive --stage wiring
    python -m studies.equity_asset_character.run_v3_drive --stage drive
"""
...
ALIAS = "SPY"
LOOKBACK_BARS = 4750
DECISIONS_DIR = Path(CHARACTER_DATASETS) / "v3-decisions"

#: U45 ∖ U30 (ledger §L2/§L13): the 19 names without stored V3 stances.
DRIVE_SYMBOLS: tuple[str, ...] = (
    "ABBV",
    "ADBE",
    "BA",
    "BAC",
...
def run_wiring(sample_windows: tuple[str, ...] = ("w05",)) -> None:
    ...
    write_json(Path(REPORT_ROOT) / "drive" / "v3_alias_wiring.json", report)
...
def run_drive(symbols: list[str], workers: int) -> None:
    ...
    write_json(Path(REPORT_ROOT) / "drive" / "v3_drive_log.json", summary)
