"""Training one study cell: symbol x window x horizon, on that window's past alone.

This is the pilot's walk-forward plan with the horizon as a parameter, and the
one rule that must move with it moved: the outer train->score gap is
``horizon + one regular session``, not the pilot's fixed 30 bars. A 26-bar
label written 30 bars before a window resolves four bars inside it; the gap
grows so no training outcome is decided by a bar the model will be graded on.

Everything the selection depends on is the shipped machinery, untouched:
``compare_candidates`` grades the same three families on anchored sub-folds
with the same 26-bar embargo and the purge that scales with the label's own
``label_knowable_at``; ``select_candidate`` applies the same 0.002 materiality
gate. Weakening that gate because it kept choosing the null is exactly what
this study is forbidden to do - the null is the finding unless a candidate
earns otherwise.

Beyond the pilot, every cell also trains and records:

- a RAW null artifact - the class-frequency estimator over all labelled
  training rows, identity calibration - the constant the out-of-sample window
  comparison is measured against;
- SHADOW artifacts for the families the selection did not choose, trained by
  the same ``train_model`` path. Shadows exist so a "the gate is masking real
  signal" claim can be tested from the record; they are marked as shadows and
  the winner rule never reads them.

Every trained model carries the calibration audit of design.md section 7.
"""
...
class CellModels:
    ...
    def to_json_dict(self) -> dict[str, object]:
        ...
        improvement = self.baseline_log_loss - self.selected_log_loss
        return {
            "symbol": self.symbol,
            "window": self.window,
...
def training_frame_for(
    ...
    *,
    ...
    """V4 training rows from ``frame[:last_row]``, labelled at `horizon_bars`.
    ...
    not allowed to know about - the pilot's rule, kept.
    """
    ...
    return build_training_frame(
        history, grid=equity_grid(sessions), label=label_spec_for(horizon_bars)
    )
...
def _null_artifact(training: TrainingFrame, *, model_version: str) -> ProbabilityArtifact:
    ...
    Identity calibration on purpose: this is the benchmark constant, not a
    shipped model, and calibrating a constant against a validation split would
    move the benchmark by the luck of that split.
    """
    ...
    return ProbabilityArtifact(
        ...
        metrics={},
        seed=STUDY_SEED,
        notes="Raw class-frequency null benchmark for the horizon study.",
    )
...
def train_cell(
    ...
    *,
    ...
    gap = outer_gap_bars(horizon_bars)
    first_scored, _ = window.positions(frame)
    last_training_row = first_scored - gap - 1
    if last_training_row < 0:
        raise CellError(
            f"{symbol}/{window.name}/h{horizon_bars}: the window opens at row "
            f"{first_scored}, which leaves no room for a {gap}-bar gap."
        )

    training = training_frame_for(
        frame, calendar, symbol=symbol, last_row=last_training_row, horizon_bars=horizon_bars
    )
    if len(training.frame) < MIN_TRAINING_ROWS:
        raise CellError(
            f"{symbol}/{window.name}/h{horizon_bars}: only {len(training.frame)} training "
            f"rows are available; at least {MIN_TRAINING_ROWS} are required."
        )
    ...
    version_stem = f"equity-h{horizon_bars}-{symbol.lower()}-{window.name}"
    trained_by_family = {}
    ...
    for candidate in default_candidates():
        trained = train_model(
            ...
            seed=seed,
            notes=(
                f"Equity V4 horizon study, horizon {horizon_bars} bars. "
                f"{'SELECTED' if candidate.family == selected.family else 'SHADOW'} for "
                f"{window.name}. Fitted on rows up to "
                f"{training.frame['feature_timestamp'].iloc[-1].isoformat()}, {gap} bars "
                f"before the window opens."
            ),
        )
...
def assert_gap_respected(cell: CellModels, frame: pd.DataFrame) -> tuple[str, ...]:
    """Confirm the cell's stored instants respect the horizon-scaled gap.

    Compares stored timestamps rather than the row arithmetic that produced
    them, the same discipline as the pilot's ``assert_no_forward_information``:
    an off-by-one in ``train_cell`` surfaces here instead of being reproduced.
    """
    problems: list[str] = []
    timestamps = pd.DatetimeIndex(frame["timestamp"])
    last_train = pd.Timestamp(cell.training_last_bar)
    first_score = pd.Timestamp(cell.scoring_first_bar)
    if last_train >= first_score:
        problems.append(
            f"{cell.symbol}/{cell.window}/h{cell.horizon_bars}: last training bar "
            f"{last_train.isoformat()} is not before the first scored bar."
        )
    ...
    if gap < cell.gap_bars:
        problems.append(
            f"{cell.symbol}/{cell.window}/h{cell.horizon_bars}: only {gap} bars separate "
            f"training from scoring; {cell.gap_bars} were required."
        )
    return tuple(problems)
