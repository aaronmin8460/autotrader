 ...
 *
 * `CLEAN`, `DEGRADED`, `MISMATCH`, `UNKNOWN`, `SELL`, `EQUITY_RUNTIME` and
 * `MANUAL_OPERATOR` are the ledger's own words and are printed identically in
 * both locales; only the labels and explanations around them are translated.
 */
      ...
      {statusWord(panel) !== "CLEAN" ? (
        <div className="border-t border-subtle px-4 py-3">
          <p className={cn("text-meta leading-snug", toneText(tone))}>
            {detail(panel)}{" "}
            {panel?.available ? t("pnl.mayBeWrong", { status: statusWord(panel) }) : ""}
          </p>
        </div>
      ) : null}
