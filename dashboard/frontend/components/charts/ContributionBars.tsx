...
export function ContributionBars({ rows }: { rows: Contribution[] }) {
  ...
  return (
    <ul className="space-y-1.5" aria-label="Unrealized P&L by position">
      {rows.map((row) => {
        ...
        return (
          <li key={row.symbol} className="grid grid-cols-[64px_minmax(0,1fr)_88px_64px] items-center gap-2">
            <span className="truncate text-[12px] font-medium text-ink">{row.symbol}</span>
            ...
            <span className={cn("num text-right text-[12px]", positive ? "text-pos" : "text-neg")}>
              {signedMoney(row.pnl)}
            </span>
            <span className="num text-right text-[11px] text-ink-3">{signedPercent(row.fraction)}</span>
          </li>
        );
      })}
    </ul>
